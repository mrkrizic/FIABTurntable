...
public class TransportModuleCoordinatorActorImpl extends TransportModuleCoordinatorActorBase {
    ...
    private InternalTransportModuleRequest currentRequest;
    ...
    @NotificationHandler
    public void handleClientEndpointStatusNotification(ClientHandshakeEndpointNotification clientHandshakeEndpointNotification) {
        log.info("Received Endpoint status from " + clientHandshakeEndpointNotification.getStatus().getCapabilityId() +
                " with value:" + clientHandshakeEndpointNotification.getStatus().getState());
        if (!hsEndpointNonUpdatableStates.contains(stateMachine.getState())) {
            this.handshakeEndpointInfo.addOrReplace(clientHandshakeEndpointNotification.getStatus());
        } else {
    ...
    private void startSourceHandshake(){
        Optional<LocalEndpointStatus> fromEP = handshakeEndpointInfo.getHandshakeEP(currentRequest.getCapIdFrom());
        if (fromEP.isPresent()) {
    ...
    private void turnToSource(InternalTransportModuleRequest request){
        this.currentRequest = request;
        Optional<LocalEndpointStatus> fromEP = handshakeEndpointInfo.getHandshakeEP(request.getCapIdFrom());
        Optional<LocalEndpointStatus> toEP = handshakeEndpointInfo.getHandshakeEP(request.getCapIdTo());
        if (fromEP.isPresent() && toEP.isPresent()) {
            ...
            if (!fromEP.isPresent())
                log.warning("Unknown HandshakeEndpoint identified by CapabilityId " + request.getCapIdFrom());
            if (!toEP.isPresent())
                log.warning("Unknown HandshakeEndpoint identified by CapabilityId " + request.getCapIdTo());
            stop();
        }
    }

    private void startDestinationHandshake(){
        log.info("Continuing with Destination Handshake via: " + currentRequest.getCapIdTo());
        Optional<LocalEndpointStatus> toEP = handshakeEndpointInfo.getHandshakeEP(currentRequest.getCapIdTo());
        toEP.ifPresent(leps -> {
