...
public class ClientHandshakeNorthActorImpl extends ClientHandshakeNorthActorBase {
    ...
    public ClientHandshakeNorthActorImpl(@Named("OpcUaClientNorthConnector") ActorRef connectorTopDomain,
        ...
        this.capabilityId = TurntableModuleWellknownCapabilityIdentifiers.TRANSPORT_MODULE_NORTH_CLIENT;
    ...
    public Receive createReceive() {
        return super.createReceive().orElse(receiveBuilder()
                .match(LocalEndpointStatus.LocalClientEndpointStatus.class, status -> {
                    log.debug("Received Client Endpoint Status: " + status.getState());
                    publishNotification(new ClientHandshakeEndpointNotification(status));
                })
                .match(HandshakeCapability.ClientSideStates.class, state -> {
                    log.debug("Received Client Side State: " + state.name());
                    publishNotification(new ClientHandshakeNotification(capabilityId, state));
                })
                .matchAny(msg -> log.debug("Received unknown message: " + msg.toString()))
                .build());
    }
