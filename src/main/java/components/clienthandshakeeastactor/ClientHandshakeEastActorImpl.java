...
import components.clienthandshakeeastactor.base.ClientHandshakeEastActorBase;
...
public class ClientHandshakeEastActorImpl extends ClientHandshakeEastActorBase {
    ...
    @Inject
    public ClientHandshakeEastActorImpl(@Named("NoConnector") ActorRef connectorTopDomain,
        ...
        this.capabilityId = TurntableModuleWellknownCapabilityIdentifiers.TRANSPORT_MODULE_EAST_CLIENT;
            ...
            info.values().stream()
                    ...
                    .forEach(wi -> {
                        try {
                            ...
                            log.warning("Error applying wiring info for id: {} - url: {}, with message:",
                                    wi.getRemoteEndpointURL(), wi.getRemoteEndpointURL(), e.getMessage());
                            e.printStackTrace();
                        }
                    });
        });
